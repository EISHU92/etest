	...
	std::map<v3s16, MapBlock*> &modified_blocks)
{
	...
	while (from_nodes.next(current_light, current)) {
		...
		source_dir = 6;
		// The current node
		const MapNode &node = current.block->getNodeNoCheck(current.rel_position, &is_valid_position);
		...
		for (direction i = 0; i < 6; i++) {
			...
			MapNode neighbor = neighbor_block->getNodeNoCheck(neighbor_rel_pos, &is_valid_position);
	...
	for (LightBank bank : banks) {
		...
		for (u8 i = 0; i <= LIGHT_SUN; i++) {
			...
			for (std::vector<ChangingLight>::const_iterator it = lights.begin();
					it < lights.end(); ++it) {
				MapNode n = it->block->getNodeNoCheck(it->rel_position, &is_valid_position);
	...
	std::map<v3s16, MapBlock*> &modified_blocks)
{
	...
	for (LightBank bank : banks) {
		...
		for (direction d = 0; d < 6; d++) {
			...
			for (u8 blocknum = 0; blocknum < 2; blocknum++) {
				...
				for (s32 y = a.MinEdge.Y; y <= a.MaxEdge.Y; y++) {
					MapNode n = b->getNodeNoCheck(x, y, z, &isvpos);
					ContentLightingFlags f = ndef->getLightingFlags(n);
					u8 light = n.getLight(bank, f);
					// Sunlight is fixed
					if (light < LIGHT_SUN) {
		...
		for (u8 i = 0; i <= LIGHT_SUN; i++) {
			...
			for (std::vector<ChangingLight>::const_iterator it = lights.begin();
					it < lights.end(); ++it) {
				MapNode n = it->block->getNodeNoCheck(it->rel_position, &isvpos);
				n.setLight(bank, i, ndef->getLightingFlags(n));
				it->block->setNodeNoCheck(it->rel_position, n);
			}
		}
	...
	const NodeDefManager *ndef, bool light[MAP_BLOCKSIZE][MAP_BLOCKSIZE])
{
	...
	if (source_block == NULL || !source_block->isGenerated()) {
		...
		for (s16 x = 0; x < MAP_BLOCKSIZE; x++) {
			...
			MapNode above = source_block->getNodeNoCheck(x, 0, z, &vpos);
			ContentLightingFlags above_f = ndef->getLightingFlags(above);
	...
	for (index = 0; index < data->data.size(); index++) {
		...
		if (it.is_sunlit) {
			...
			for (; current_pos.Y >= 0; current_pos.Y--) {
				MapNode n = block->getNodeNoCheck(current_pos, &vpos);
				ContentLightingFlags f = ndef->getLightingFlags(n);
	...
	std::map<v3s16, MapBlock*> *modified_blocks)
{
	...
	for (blockpos.Z = minblock.Z; blockpos.Z <= maxblock.Z; blockpos.Z++) {
		...
		for (relpos.Y = 0; relpos.Y < MAP_BLOCKSIZE; relpos.Y++) {
			MapNode node = block->getNodeNoCheck(relpos.X, relpos.Y, relpos.Z, &vpos);
	...
	for (size_t b = 0; b < 2; b++) {
		...
		for (u8 i = 0; i <= maxlight; i++) {
			...
			for (std::vector<ChangingLight>::const_iterator it = lights.begin();
					it < lights.end(); ++it) {
				MapNode n = it->block->getNodeNoCheck(it->rel_position, &vpos);
				n.setLight(bank, i, ndef->getLightingFlags(n));
				it->block->setNodeNoCheck(it->rel_position, n);
			}
		}
	...
	for (blockpos.Z = minblock.Z; blockpos.Z <= maxblock.Z; blockpos.Z++) {
		...
		for (const VoxelArea &a : block_pad) {
			...
			for (relpos.Y = a.MinEdge.Y; relpos.Y <= a.MaxEdge.Y; relpos.Y++) {
				...
				MapNode oldnode = block->getNodeNoCheck(relpos, &vpos);
	...
	bool light[MAP_BLOCKSIZE][MAP_BLOCKSIZE])
{
	...
	for (s16 x = 0; x < MAP_BLOCKSIZE; x++) {
		...
		for (s16 y = MAP_BLOCKSIZE - 1; y >= 0; y--) {
			MapNode n = block->getNodeNoCheck(x, y, z, &vpos);
			// Ignore IGNORE nodes, these are not generated yet.
			if (n.getContent() == CONTENT_IGNORE)
				continue;
			ContentLightingFlags f = ndef->getLightingFlags(n);
	...
	std::map<v3s16, MapBlock*> *modified_blocks)
{
	...
	for (const VoxelArea &a : block_pad) {
		...
		for (relpos.Y = a.MinEdge.Y; relpos.Y <= a.MaxEdge.Y; relpos.Y++) {
			...
			// Get node
			MapNode node = block->getNodeNoCheck(relpos, &vpos);
			ContentLightingFlags f = ndef->getLightingFlags(node);
			// For each light bank
			for (size_t b = 0; b < 2; b++) {
				LightBank bank = banks[b];
